...
test('permission policy is persisted, revisioned, audited, and snapshot into sessions', () => {
  ...
  const migration = readFileSync('prisma/migrations/20260911010000_add_role_permissions/migration.sql', 'utf8');
  ...
  for (const [role, grants] of Object.entries(DEFAULT_ROLE_PERMISSIONS)) {
    for (const permission of grants) assert.match(migration, new RegExp(`\\('${role}', '${permission}'\\)`));
  }
...
test('backup version 5 includes permissions while legacy restores preserve them', () => {
